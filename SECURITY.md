...
3. **OpenCode Zen free models may log or train during their free period.** `--worker opencode --model free` ranks the strongest listed-free coding model. Treat free Zen models like the easy lane for secrets. Pin `CURSOR_ROUTE_OPENCODE_MODEL` for a frozen id. Ox Alpha (`opencode/x-preview-f-free`) remains the zero-retention **fallback** option.
